...
export function MajorManagementPanel({ initialItems, canManage }: { readonly initialItems: readonly MajorView[]; readonly canManage: boolean }) {
  ...
  const form = (mode: "create" | "edit") => { const value = mode === "edit" ? editing : null; return <form onSubmit={mode === "create" ? submitCreate : submitEdit}><FieldGroup className="grid md:grid-cols-3 xl:grid-cols-4"><Field><FieldLabel htmlFor={`${mode}-major-code`}>Mã ngành</FieldLabel><Input id={`${mode}-major-code`} name="code" defaultValue={value?.code} required maxLength={50} /></Field><Field><FieldLabel htmlFor={`${mode}-major-name`}>Tên ngành</FieldLabel><Input id={`${mode}-major-name`} name="name" defaultValue={value?.name} required maxLength={255} /></Field><Field><FieldLabel htmlFor={`${mode}-major-order`}>Thứ tự</FieldLabel><Input id={`${mode}-major-order`} name="displayOrder" type="number" min={0} step={1} defaultValue={value?.displayOrder ?? 0} required /></Field><div className="flex items-end gap-2"><Button type="submit" disabled={pending}>{pending ? <Spinner data-icon="inline-start" /> : mode === "create" ? <PlusIcon data-icon="inline-start" /> : null}{mode === "create" ? "Tạo ngành" : "Lưu"}</Button>{mode === "edit" && <Button type="button" variant="outline" onClick={() => setEditing(null)}>Hủy</Button>}</div></FieldGroup></form>; };
